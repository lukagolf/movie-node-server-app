...
DROP PROCEDURE IF EXISTS submit_report;
 DELIMITER $$
 CREATE PROCEDURE submit_report(IN rep_category ENUM, IN sub_date DATETIME, IN sub_id INT, IN aid INT, IN rid INT, 
...
DROP FUNCTION IF EXISTS getCategoryReports;
DELIMITER $$
CREATE FUNCTION getCategoryReports(report_category ENUM)
RETURNS TABLE
...
DROP FUNCTION IF EXISTS viewReportsByDate;
DELIMITER $$
...
DROP FUNCTION IF EXISTS getSubmitterReports;
DELIMITER $$
...
DROP FUNCTION IF EXISTS getAdminReports;
DELIMITER $$
...
-- Returns: A table containing the reports that are not resolved.
